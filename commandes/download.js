...
zokou({nomCom : "igdl" , categorie : "Download"},async (dest , zk , commandeOptions)=>{
  ...
  try {
    ...
    zk.sendMessage(dest,{video : {url : igvid.data.data.data[0].url},caption : "ig video downloader powered by *𝐌𝐀𝐍𝐔-𝐌𝐃*",gifPlayback : false },{quoted : ms}) 
    }
    else {
        zk.sendMessage(dest,{image : {url : igvid.data.data.data[0].url},caption : "ig image downloader powered by *𝐌𝐀𝐍𝐔-𝐌𝐃*"})
    }
  ...
  try {
    ...
    .then((result) => {
      ...
      `;
       ...
       zk.sendMessage(dest, { video: { url: result.hd  }, caption: 'facebook video downloader powered by *𝐌𝐀𝐍𝐔-𝐌𝐃*' }, { quoted: ms });
...
async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    .then((result) => {
      ...
      `;
       zk.sendMessage(dest,{image : { url : result.thumbnail}, caption : caption},{quoted : ms}) ;
       zk.sendMessage(dest, { video: { url: result.sd  }, caption: 'facebook video downloader powered by *𝐌𝐀𝐍𝐔-𝐌𝐃*' }, { quoted: ms });
